        ...
        for l in k.find_all('li'):
            for m in l.find_all('a'):
                if m['href'].startswith('/en/list/') and m.has_attr('style')==False:
                    ...
                    web_data['property_category'].append(''.join(re.findall('<li class="(.+?)"', str(l))))
                            
df_unesco_list = pd.DataFrame(web_data)
df_unesco_list.to_csv(f'{path_output_csv}/unesco_world_heritage_sites.csv', header=True, index=False, encoding='utf-8-sig',sep=';')
